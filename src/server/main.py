...
def init_db():
    ...
    if not c.fetchone():
        c.execute(
            "INSERT INTO global_stats (id, tokens, chunks, runs) VALUES (1, 0, 0, 0)"
        )
    conn.commit()
    conn.close()
...
async def cook_text(request: CookRequest):
    ...
    tokens_count = len(request.text)  # Quick approx or use tiktoken if available
    return {"message": "Python Cooker Ready", "input_length": tokens_count}
